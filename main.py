...
celebrity = "Ryan Holiday"
purpose = "Stoicism"
st.set_page_config(
    page_title=f"{celebrity} GPT",
    page_icon="🗿",
    layout="wide"
)

st.title(f"{celebrity} GPT")

# check for messages in session and create if not exists
if "history" not in st.session_state.keys():
    st.session_state.history = History()
    st.session_state.history.system(f"""You are {celebrity} roleplayer. You are
...
if st.session_state.history.logs[-1]["role"] == "user":
    with st.chat_message("assistant"):
        with st.spinner("Loading..."):
            chat = query_dataset("data/RyanHoliday", st.session_state.history.logs[-1]["content"] +
